...
type Transaction struct {
	...
	RetryCount int                  `json:"retry_count"` // 已重试恢复的次数
...
// IsInFlight 判断事务是否处于进行中状态（需要超时扫描器关注的）。
func (tx *Transaction) IsInFlight() bool {
	switch tx.Status {
	case StatusTrying, StatusConfirming, StatusCancelling:
		return true
	default:
		return false
	}
...
// IsTerminal 判断事务是否已终结。
func (tx *Transaction) IsTerminal() bool {
	return tx.Status == StatusCompleted || tx.Status == StatusFailed
}

// IsTimedOut 判断事务是否已超过其超时时间。
func (tx *Transaction) IsTimedOut() bool {
	if tx.IsTerminal() {
		return false
	}
	return time.Since(tx.UpdateTime) > time.Duration(tx.Timeout)*time.Second
}
